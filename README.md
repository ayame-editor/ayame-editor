...
curl -fsSL https://raw.githubusercontent.com/hjosugi/ayame-editor/main/scripts/install.sh \
  | AYAME_VERSION=0.1.17 AYAME_INSTALL_DIR="$HOME/.local/bin" sh
```
...
**メモ帳風の編集**：行をクリックするとその位置にカーソルが入り、そのまま入力できます。`Enter` で行を分割、`Backspace`（行頭）/`Delete`（行末）で前後の行と結合、`↑`/`↓`/`←`/`→` で行をまたいでカーソル移動、**複数行の貼り付け**はそのまま複数行に展開されます。行末には `[EOF]` を表示します。

**範囲選択**：ドラッグ（または `Shift`＋クリック）で**複数行にまたがる選択**ができ、`Ctrl+C`／`Ctrl+X` でコピー／カット、`Ctrl+A` で全選択、`Delete`／`Backspace` で選択範囲を削除できます。
