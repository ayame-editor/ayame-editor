...
> `ayame-diff sorted`、Web GUI 付き）が担当します。削除されたコマンドと UI の
> 置き換えは [移行ガイド](docs/ja/DIFF_MIGRATION.md) を参照してください。
...
- CLI では `stat`、`head`、`tail`、`line`、`lines`、`search`、`sort`、`replace`、`case`、`grep-lines`、`split`、`group`、`top`、`distinct`、`gen`、`serve`、`cache`、`update`、`remove` などを使えます。
