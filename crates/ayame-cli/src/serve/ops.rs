...
static WORKER_EXE: OnceLock<Result<PathBuf, String>> = OnceLock::new();
...
pub(super) fn initialize_worker_executable() -> anyhow::Result<()> {
    match WORKER_EXE.get_or_init(|| {
        let current = std::env::current_exe().map_err(|error| error.to_string())?;
        let dir =
            temp_paths::create_private_temp_dir("srv-worker").map_err(|error| error.to_string())?;
        let name = if cfg!(windows) { "ayame.exe" } else { "ayame" };
        ...
        Ok(_) => Ok(()),
        Err(message) => anyhow::bail!("preparing stable worker executable: {message}"),
    }
}

pub(super) fn cancel_operations_and_cleanup_worker() {
    if let Some(map) = ARTIFACT_OPS.get() {
        for operation in map.lock().unwrap().values() {
            operation.canceled.store(true, AtomicOrdering::Relaxed);
            operation.notify_cancel.notify_waiters();
        }
    }
    if let Some(Ok(worker)) = WORKER_EXE.get() {
...
fn worker_command() -> Result<Command, ApiError> {
    initialize_worker_executable().map_err(internal)?;
    let exe = WORKER_EXE
        .get()
        .and_then(|result| result.as_ref().ok())
        .ok_or_else(|| internal("stable worker executable was not initialized"))?;
    if !exe.is_file() {
        return Err(ApiError::new(
            StatusCode::CONFLICT,
            "restart_required",
            "the worker executable is unavailable; restart Ayame",
        ));
    }
    Ok(Command::new(exe))
}
