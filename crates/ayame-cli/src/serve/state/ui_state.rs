...
use super::AppState;
use crate::serve::analysis::AnalysisProfile;
use crate::serve::{internal, ApiError};
...
impl AppState {
    ...
    pub(in crate::serve) fn load_ui_state(&self) -> UiState {
        ...
        (ui.analysis_profiles, ui.active_analysis_profile) =
            crate::serve::analysis::sanitize_persisted_profiles(
    ...
    pub(in crate::serve) fn save_session_snapshot(&self) -> Result<UiState, ApiError> {
        ...
        ui.session.paths = tabs
            .iter()
            .map(|t| t.path.clone())
            .filter(|p| !crate::serve::workspace::is_scratch_path(p))
            .collect();
...
pub(super) const SESSION_MAX_PATHS: usize = 512;

pub(super) fn clean_string_list(list: Vec<String>, max: usize) -> Vec<String> {
    let mut out = Vec::new();
    for value in list {
