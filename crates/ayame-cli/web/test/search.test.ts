...
describe("matcher construction (#129)", () => {
  ...
  it("escapes literal queries and applies case-insensitive flags", () => {
    ...
    expect(state.search.matcher?.flags).toContain("i");
    expect("A+B".match(state.search.matcher || /$^/)).toEqual(["A+B"]);
  ...
  it("falls back from the Unicode whole-word wrapper when the plain regex is valid", () => {
    ...
    expect(state.search.regexError).toBe(false);
    expect(state.search.matcherWordFallback).toBe(true);
    expect(state.search.matcher?.flags).toBe("g");
  });
  ...
  it("marks a regex invalid when both wrapped and plain forms fail", () => {
    ...
    expect(state.search.matcher).toBeNull();
    expect(state.search.regexError).toBe(true);
...
describe("find navigation wrapping (#129)", () => {
  it.each([
    ...
    expect(state.search.lastMatch).toEqual({ byte: hit.byte, len: hit.byte_len });
    expect(state.caret.position.line).toBe(hit.line);
...
describe("search count request generation (#123)", () => {
  it("aborts an older request and ignores its late response", async () => {
    ...
    await newCount;
    expect(state.search.hits).toEqual([{ byte: 2 }]);
