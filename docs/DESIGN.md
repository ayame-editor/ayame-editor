...
**正直なトレードオフ:** 各軸は既存ツールが個別に解決済み。Ayame の価値は**統合・パッケージング**にある（技術的moatは薄い）。よってスコープを「ビューア＋データ操作」に限定し、EmEditor級の248GB**編集**には踏み込まない（成熟したtemp-file編集エンジンと正面衝突して負けるため）。
...
- **編集機能とその編集WAL**。今回のスコープは明示的に**ビューア＋ops**で、編集は未着手。設計もロードマップ入りまで書かない。

---

## 11. 未解決の問い（要追加検討）

1. 北極星（10^10行/TB級）の**実測検証**: 合成 50GB/10億行 JSONL を EmEditor(Win VM)/klogg/lnav/DuckDB で計測し、「GUI＋ops＋O(index)メモリ＋OSS」の同時成立が他に無いことをエビデンス化。
