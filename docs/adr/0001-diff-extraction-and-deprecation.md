...
v0.6.0 で非推奨化を告知し、その後の削除作業で実装、HTTP endpoint、Web/native
UI、関連テストを撤去した。旧 CLI 名は 1 リリースの間、ayame-diff への移行案内を
返す。利用者向けの対応表は [diff 移行ガイド](../ja/DIFF_MIGRATION.md) に記載する。
