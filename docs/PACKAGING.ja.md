...
署名導入前のリリースには `.sha256.sig` がないため、鍵を持つビルドはそれらを拒否
します。これは `--version` にも及びます。v0.10.0 以降のビルドから
`ayame update --version v0.9.0` で戻そうとすると、「release v0.9.0 is not signed」
として非ゼロ終了し、何もインストールされません。これはポリシーが働いている状態で
あり、不具合ではありません。回避フラグは意図的に用意していません — 未検証の成果物を
インストールするフラグは、署名が防ごうとしているものそのものだからです。
巻き戻す場合は該当リリースの成果物を手動でダウンロードしてください。
