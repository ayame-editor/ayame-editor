...
- **既知の限界**: reset/コンパクション地点より古い履歴への undo/redo は
  全量スナップショットに退化する(復旧後の undo 履歴は「ログ開始以降」のみ)。
  保存後のスナップショットは保存時点のオーバーレイを捕捉して**新基底へ
  リベース**されるため、undo で保存点を跨いでも復元内容はズレない
  (`reset_for_save`)。旧来の plain reset 経路に落ちた場合はログを
  クリーン化して復旧を無効化する(誤った復元は構造的に起きない)。
  改行終端まで揃った破損行は Invalid としてログ全体の復旧を拒否し、
  EOF の書きかけ断片のみ無視する。
