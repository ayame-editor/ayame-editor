...
- **インクリメンタル索引**（構築中の先頭から閲覧開始）。tail -f 追従は実装済み（ログ自動追従）。

## 🚧 意図的に「やらない／後回し」

- **編集WALの永続化**（append-only ログ / piece table のディスク保存）。行単位差分編集、undo/redo（保存跨ぎ含む）、上書き/別名保存、範囲・矩形選択、選択範囲置換、マルチカーソル、全体置換/ケース変換/ソート/分割まで実装済みで、未着手はこの永続化のみ。**フルインメモリ rope は作らない**。
